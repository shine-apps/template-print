...
| §10 M1 分期条目（旋转/吸附/分类管理/导入导出/状态检查属 M2；条码已移出产品范围） | 全部任务 |

**M2 明确排除项**（本计划不实现，验收时不得当作缺陷）：元素旋转、辅助线与网格吸附、图层拖拽排序（仅按钮置顶）、模板分类管理 UI（分类字段已存储，列表仅按已存在分类过滤）、`.tplx` 导入导出、PowerShell 打印机状态检查、图片 fit/opacity 属性编辑（模型已支持）。
**已从产品范围移除（2026-09-23 决策）**：条码/二维码生成器——改由图片元素上传条码/二维码图片满足，任何版本都不实现内置生成。
