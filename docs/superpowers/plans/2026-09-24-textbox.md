...
export function layoutText(
  ...
  for (const par of paragraphs) {
    const built = buildLines(par, vertical ? boxH : boxW, st, vertical)
    rawLines.push(...built)
  }
...
`npx vitest run`（全绿，新增 text-layout 11 + font-service 2 + 模型/渲染新增若干）、`npm run typecheck` 0、`npx electron-vite build` 成功。

- [ ] **Step 2: Chromium flex 方向实测（实现前提验证）**

用最小 HTML（dataURL 或临时 print 窗口）确认 vertical-rl 下 `flex-direction:row-reverse` + `justify-content:flex-start` 时列组贴右、flex-end 贴左、center 居中；若与假设相反，调整 renderTextHtml 映射与对应测试。

- [ ] **Step 3: CDP 走查**

1. 新建横排文本框（fontFamily 默认 ''）：字体下拉显示系统默认/常用/全部三组；选具体字体后画布与打印 HTML 均生效；
2. B/I/U：下划线在画布显示且打印 HTML 含 text-decoration；
3. 新建竖排文本框，输入"甲乙AB2026"：汉字正立成列、首列贴右、英数旋转；拖宽/拖高改变换列；align 三种对齐方向正确；
4. 中文长文本横排逐字换行不溢出框；预览 iframe 与画布一致；
5. v2 旧模板（用户"测试"、内置示例）打开方向为横排、无下划线，外观不变；
6. 全应用零 console 异常。

- [ ] **Step 4: 打包与收尾**

`npm run dist`；确认 win32-x64 prebuild 存在、安装包生成；更新设计文档/项目记忆（v3 落地状态）。
